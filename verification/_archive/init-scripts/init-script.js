(() => {
  ...
  // Stub Vue and moment - 接受任意方法调用，返回合理默认值
  ...
  // 暴露 test API 入口（user.js 内部函数会通过 window.__fqa 暴露）
  window.__fqaTest = {
    registered: false,
    error: null,
  };
...
`);
  ...
  try{mainInit();}catch(e){console.error("[fqa-test] mainInit failed:",e);}
;window.__fqa={registerDevice,signRequest,registerKey,buildQuery,getChapterViaSnssdk,decryptChapter,sm3,defaultUnidbgConfig,REGISTER_URL,READING_BASE,getCrypto};
})(Vue, moment);