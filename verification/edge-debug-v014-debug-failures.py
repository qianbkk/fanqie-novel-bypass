# -*- coding: utf-8 -*-
"""Diagnose why real item_ids (Ch100/190/500/981) FAIL while synthetic PASS."""
import json
...
def main():
    ...
    (async () => {
      ...
      try {
        ...
        console.error('[edge-debug-fail] precompute failed:', e);
      }
    })();
    ...
    for label, item_id in REAL:
        ...
        for wait in [5, 12, 20]:
            time.sleep(wait if wait == 5 else (wait - (5 if wait == 12 else 8)))
            ...
            return {
              ...
              snssdkRecent: (window.__snssdkCalls || []).slice(-8),
            ...
            print(f"\n[wait={wait}s] url={result['url']}")
            print(f"  title={result['title']}")
            print(f"  bodyTextLen={result['bodyTextLen']} hasChapter={result['bodyHasMockChapter']} fqa={result['fqaElementCount']}")
            print(f"  rejected={result['importStats']['rejected']} accepted={result['importStats']['accepted']} snssdk={result['snssdkCalls']}")
            print(f"  preview: {repr(result['bodyTextPreview'][:200])}")
            if result['snssdkCalls'] > 0:
                print(f"  last snssdk calls: {result['snssdkRecent']}")
