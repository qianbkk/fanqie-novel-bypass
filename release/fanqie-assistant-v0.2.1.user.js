...
(function (vue, moment) {
  ...
  async function getChapterViaWeb(itemId2) {
    var _a;
    const url = `https://fanqienovel.com/api/reader/full?itemId=${encodeURIComponent(itemId2)}`;
    const { status, zhal, body } = await new Promise(
      (resolve, reject) => {
        ...
        xhr.onerror = () => reject(new Error(`xhr network error for ${itemId2}`));
        xhr.send();
      }
    );
    if (status !== 200) {
    ...
    if ((j == null ? void 0 : j.code) !== 0 || !((_a = j == null ? void 0 : j.data) == null ? void 0 : _a.chapterData)) {
      throw new Error(`web /api/reader/full returned code=${j == null ? void 0 : j.code} message=${(j == null ? void 0 : j.message) ?? ""}`);
    }
  ...
  async function getChapterViaSnssdk(itemId2, _retry) {
    var _a, _b;
    ...
    if (!j) {
      ...
      return await getChapterViaSnssdk(itemId2, _retry + 1);
    }
  ...
  async function getChapter(itemId2) {
    try {
      const web = await getChapterViaWeb(itemId2);
      ...
      warn("content", "同源路径失败，fallback snssdk", {
        itemId: itemId2,
        error: String((webErr == null ? void 0 : webErr.message) ?? webErr)
      });
      try {
        const snssdk = await getChapterViaSnssdk(itemId2);
        if (snssdk) {
        ...
        warn("content", "snssdk 路径也失败", {
          itemId: itemId2,
          error: String((snssdkErr == null ? void 0 : snssdkErr.message) ?? snssdkErr)
        });
        return null;
      }
    }
...
`;
  ...
  unsafeWindow.XMLHttpRequest = class XMLHttpRequest2 extends originalXMLHttpRequest {
    constructor() {
      super(...arguments);
...
`).join("");
  ...
  function decryptFontIn(root, fontId) {
